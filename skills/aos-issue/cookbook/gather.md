...
- **source**: GitHub URL from the `source` field in library.yaml, or `https://github.com/ExampleCo/aos` for aos-internal skills
...
2. **No match at all** — ask the user to clarify which component they mean. If it's not a library component, it may still be a general aos issue — proceed with type `general` and source `https://github.com/ExampleCo/aos`.
