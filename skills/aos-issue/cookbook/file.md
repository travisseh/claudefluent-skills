...
```bash
gh issue list -R Example Company/aos --search "<component_name> <key_problem_words>" --state open --limit 10 --json number,title,url
```
...
- If comment on existing: compose a comment using this template and post it:
  ```bash
  gh issue comment <number> -R Example Company/aos --body "$(cat <<'COMMENT_EOF'
  ## Additional Report
...
```bash
gh issue create -R Example Company/aos \
  --title "<title>" \
  --label "aos-issue" \
  --body "$(cat <<'ISSUE_EOF'
<body>
...
```bash
gh label create "aos-issue" -R Example Company/aos --description "Filed by aos-issue skill" --color "c2e0c6"
```
