---
name: askelephant
description: Search AskElephant meetings and transcripts, especially the user's meetings. Use when the user asks about AskElephant, AE meetings, meeting transcripts, promises, follow-ups, objections, summaries, or things discussed in recent calls.
---
...
- Default person: `user@example.com`
...
- Prefer meetings where the user is owner, host, internal participant, or appears in transcript metadata.
...
```bash
python3 ~/.codex/skills/askelephant/scripts/search_meetings.py --person user@example.com --date yesterday --query "promise promised follow up send I'll I will"
```

For a range:

```bash
python3 ~/.codex/skills/askelephant/scripts/search_meetings.py --person user@example.com --since 2026-05-06 --until 2026-05-07 --query "pricing objection competitor"
```

To fetch broader context without keyword filtering:

```bash
python3 ~/.codex/skills/askelephant/scripts/search_meetings.py --person user@example.com --date yesterday --limit 20
```
...
- `~/Programming/exampleco-reporting/apps/dashboard/.env.local`
- `~/.config/render/product-analytics.env`

## Answering Follow-Up / Promise Questions

1. Search yesterday's meetings for the user with promise/action keywords.
