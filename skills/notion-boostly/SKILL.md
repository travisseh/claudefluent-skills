---
name: notion-exampleco
description: Create, read, and update pages in the ExampleCo Notion workspace. Use when the user asks to publish docs, PRDs, roadmaps, meeting notes, or any content to ExampleCo Notion — or to search/read existing ExampleCo pages. Separate from /notion-backlog (which is scoped to the personal Life Backlog DB).
---

# Notion ExampleCo Workspace

General-purpose Notion tool for the **ExampleCo workspace**. Unlike `/notion-backlog`, this is not tied to a database — it operates on arbitrary pages and hierarchies so you can create PRDs, docs, roadmaps, and nested child pages.
...
**Always use the CLI at `~/.config/notion-tools/notion-exampleco.js`. Do NOT use the Notion MCP — it is unreliable for this workspace. Do NOT use the Life Backlog CLI (`notion.js`), which points at the personal workspace.**

```bash
node ~/.config/notion-tools/notion-exampleco.js <command> [args]
```
...
Default dumping ground for AI-generated artifacts in ExampleCo Notion.
...
```bash
node ~/.config/notion-tools/notion-exampleco.js db-add 33bc831c671e8137a524f62347485d71 "Tapcards onboarding PRD" "prds"
```
...
### Creating a new doc in ExampleCo
...
```bash
ROOT=$(node ~/.config/notion-tools/notion-exampleco.js create <examplecoParentId> "Q2 Planning" @/tmp/overview.md | jq -r .id)
SUB=$(node ~/.config/notion-tools/notion-exampleco.js create $ROOT "Workstream: Tapcards" @/tmp/tapcards.md | jq -r .id)
node ~/.config/notion-tools/notion-exampleco.js create $SUB "Rollout Notes" @/tmp/rollout.md
```
...
- **Don't confuse with `/notion-backlog`** — that skill writes to the user's personal Life Backlog database. This one writes to the ExampleCo workspace. Keep them separate.
