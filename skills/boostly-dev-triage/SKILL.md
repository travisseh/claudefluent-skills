---
name: boostly-dev-triage
description: >
  Triage Example Company Dev Triage Linear issues by reading the Linear board, checking
  for CORE duplicates, identifying the account/location from the request,
  investigating V2 production data with the v2-database skill, inspecting the
  Example Company repo, and deciding whether each issue is a bug, feature request,
  misunderstanding, data question, duplicate, or CSM-solvable workaround. Use
  when asked to process, clean up, resolve, investigate, or comment on Example Company
  Dev Triage issues.
---

# Example Company Dev Triage

Use this skill to reduce Example Company Dev Triage issues to the cheapest correct
outcome. Do not assume every ticket needs engineering work. Prefer resolving
with a clear answer, duplicate link, CSM action, or narrowed feature request
when that is enough.

## Required Inputs And Tools

- Use the Linear app/tools to inspect the Dev Triage board and issues.
- Use the `v2-database` skill for production V2 account, location, contact,
  campaign, messaging, widget, tapcard, TFV, and activity evidence.
- Inspect the local Example Company repo before claiming a bug is likely code-backed.
...
3. Identify the likely account and location from the request. Look for explicit
   ...
   campaign names, user emails, or CSM comments.
4. Search Linear for duplicates in the CORE project before deeper work. Search
...
For CSM-solvable workarounds, name the manual action clearly. Examples:

- Move the customer from legacy webform to the V2 subscribe widget.
...
Inspect the Example Company repo to connect symptoms to implementation. Use `rg` first.
...
- Legacy/V1 vs V2 paths, especially subscribe links/widgets, webforms,
...
Duplicate check: <none found / possible duplicate CORE-123 / duplicate of CORE-123>

Evidence:
- <V2 DB fact with relevant IDs/timestamps/counts>
...
- <close with answer / link duplicate / CSM manual action / create feature request / engineering fix>

CSM follow-up needed:
...
or "why does this number/state look like this?" and can be answered from V2 data
without code changes.

Treat it as duplicate when an existing CORE issue covers the same customer
...
- Existing workaround: can CSM/admin resolve it today?
...
- Ownership: should this stay in CORE, be linked to another team/project, or be
